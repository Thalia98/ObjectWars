...
class Test_integracion():
    ...
    def test_daño_al_jugador(self):
        ...
        assert(N_VIDAS-miSoldado.puntos_ataque ==
...
class Test_usuario():
    def test_integracion_a_implementar(self):
        ...
        miCaballero = Caballero()
        miJuego.jugador2.unidades.append(miCaballero)

        miJuego._batalla()
        assert (miJuego.jugador1.puntos_vida == 15)
        assert (miJuego.jugador2.puntos_vida == 20)

    def test_unitario_a_implementar(self):
        mi_caballero = Caballero()
        max_puntos_vida = mi_caballero.puntos_vida = mi_caballero.puntos_vida - 1
        mi_caballero.descansar()

        assert(max_puntos_vida == mi_caballero.puntos_vida)

    def test_funcional_a_implementar(self, monkeypatch):
        sol_file_path = 'test/test_comprar_caballeros.out'
        test_path = 'test/test.out'
        with open(test_path, 'w', encoding="utf-8") as file:
            sys.stdout = file
            Juego.mensaje_bienvenida()
